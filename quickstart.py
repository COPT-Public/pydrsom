...
parser.add_argument("--batch", required=False, type=int, default=128, help="batch size")
parser.add_argument(
    "--epoch", required=False, type=int, default=5, help="epoch number to run"
)
parser.add_argument(
...
def train(dataloader, name, model, loss_fn, optimizer, args):
    if name.startswith("drsom"):
        return train_drsom(dataloader, name, model, loss_fn, optimizer, args.interval)
    st = time.time()
...
if __name__ == "__main__":
    ...
    method_kwargs = {
        "adam": dict(lr=0.001, betas=(0.99, 0.999)),
        "sgd1": dict(lr=0.001),
        "sgd2": dict(lr=0.001, momentum=0.9),
        "sgd3": dict(lr=0.001, momentum=0.95),
        "sgd4": dict(lr=0.001, momentum=0.99),
        "lbfgs": dict(line_search_fn="strong_wolfe", max_iter=args.itermax),
    ...
    writer = SummaryWriter(
        log_dir=os.path.join(f"{args.tflogger}-{args.seed}", log_name)
    )
    ...
    for t in range(start_epoch, start_epoch + args.epoch):
        try:
            ...
            _, avg_loss, acc = train(
                train_dataloader, name, model, loss_fn, optimizer, args
            )
