...
@note:
  ...
  - we compute HVPs for its own blocks
"""
...
class DRSOMK(torch.optim.Optimizer):
  ...
  def __init__(
    ...
    super(DRSOMK, self).__init__(params, defaults)
    ##########################
    # AD hvps
    ##########################
    self.Hv = None
  ...
  def get_name(self):
    return f"drsom-k:d@{DRSOM_MODE}.{self.option_tr}:ad@{DRSOM_MODE_HVP}"
